...
(function()
{
	...
	*/
	...
	function syncFieldsBetweenTabs( currentPageId )
	{
		...
		sourceIndex = currentPageId === 'pfxFind' ? 1 : 0;
	...
	var findDialog = function( editor, startupPage )
	{
		...
		return {
							...
							[
								{
									...
									onClick : function()
									{
										...
										if ( !finder.find( dialog.getValueOf( 'pfxFind', 'txtFindFind' ),
													dialog.getValueOf( 'pfxFind', 'txtFindCaseChk' ),
													dialog.getValueOf( 'pfxFind', 'txtFindWordChk' ),
													dialog.getValueOf( 'pfxFind', 'txtFindCyclic' ) ) )
											alert( lang
												.notFoundMsg );
									}
								}
							...
							[
								{
									...
									onClick : function()
									{
										...
										if ( !finder.replace( dialog,
													dialog.getValueOf( 'pfxReplace', 'txtFindReplace' ),
													dialog.getValueOf( 'pfxReplace', 'txtReplace' ),
													dialog.getValueOf( 'pfxReplace', 'txtReplaceCaseChk' ),
													dialog.getValueOf( 'pfxReplace', 'txtReplaceWordChk' ),
													dialog.getValueOf( 'pfxReplace', 'txtReplaceCyclic' ) ) )
											alert( lang
												.notFoundMsg );
									}
								}
				...
				this.selectPage = CKEDITOR.tools.override( this.selectPage, function( originalFunc )
					{
						return function( pageId )
						{
							...
							patternFieldId = pageId === 'pfxFind' ? 'txtFindFind' : 'txtFindReplace';
							wholeWordChkFieldId = pageId === 'pfxFind' ? 'txtFindWordChk' : 'txtReplaceWordChk';
			...
			onFocus : function()
			{
				if ( startupPage == 'pfxReplace' )
					return this.getContentElement( 'pfxReplace', 'txtFindReplace' );
				else
					return this.getContentElement( 'pfxFind', 'txtFindFind' );
			}
		};
	};

	CKEDITOR.dialog.add( 'pfxFind', function( editor )
		{
			return findDialog( editor, 'pfxFind' );
		});

	CKEDITOR.dialog.add( 'pfxReplace', function( editor )
		{
			return findDialog( editor, 'pfxReplace' );
		});
})();
