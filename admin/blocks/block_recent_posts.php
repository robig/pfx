 ...
 *
 * Title: Recent blog/dynamic/news posts block
 *
 * @package PFX
...
?>
	<div id="block_recent_posts" class="block">
		...
		<div class="block_body">
			<ul>
				<?php
				$pfx_r1 = safe_rows('*', 'pfx_dynamic_posts', "post_id >= '1' and posted < now() and public = 'yes' order by posted desc limit {$pfx_post_limit}");
					...
					if ($pfx_r1) {
						$pfx_i = 0;
							while ($pfx_i < $pfx_total){
								...
								echo "\t\t\t\t\t\t\t<li><a href=\"" . createURL($pfx_page, 'permalink', $pfx_post_slug) . "\">{$pfx_title}</a> <span class=\"recent_date\">{$pfx_date}</span></li>\n";
								$pfx_i++;
							}
					}
				?>
